...
// ⭐️ DiaryEntry는 JournalResponse를 확장 ⭐️
...
// ⭐️ GrammarFeedbackView의 Props 타입을 명시적으로 정의 ⭐️
interface GrammarFeedbackViewProps {
    setCurrentView: (view: HomeViewScreen) => void; 
    // GrammarFeedbackView가 useJournalStore의 currentJournalId를 사용하여 데이터를 가져온다고 가정
}

// ⭐️ GrammarFeedbackView 컴포넌트 변환 (오류 해결 핵심) ⭐️
const GrammarFeedbackView = GrammarFeedbackViewOriginal as React.FC<GrammarFeedbackViewProps>;

// ⭐️ 텍스트 자르기 헬퍼 함수 및 최대 길이 정의 ⭐️
...
const JournalListView: React.FC<JournalListViewProps> = ({ 
    ...
    const renderPageNumbers = () => {
        ...
        // 끝 그룹이 MAX_VISIBLE_PAGES보다 작으면 시작 그룹 조정
    ...
    return (
        <View style={listStyles.container}>
            ...
            <ScrollView 
                ...
                // ⭐️ ScrollView 콘텐츠 컨테이너의 최소 높이 확보 (하단 버튼 위치 고정) ⭐️
                contentContainerStyle={[listStyles.scrollViewContent, { 
                    flexGrow: 1, // 최소 높이 확보
                    justifyContent: 'space-between', // 콘텐츠가 적을 때 상단과 하단에 붙도록
                }]}
            >
                {isPagingLoading && journals.length === 0 ? (
                    ...
                    <View style={listStyles.journalList}>
                        {journals.map((entry, index) => (
                            <DiaryEntryItem 
                                key={entry.journalId} 
                                // ⭐️ isJournalListItem 플래그를 사용하여 JournalListView 전용 스타일 적용 ⭐️
...
const listStyles = StyleSheet.create({
    ...
    scrollViewContent: {
        paddingBottom: 50, // 하단 패딩
    },
    journalList: {
        paddingTop: 10,
        gap: 10, // ⭐️ 일기 항목 간 간격 10px ⭐️
    },
    journalListEmpty: { // 데이터 없을 때 사용하는 컨테이너 (flex: 1)
        flex: 1,
...
const HomeView: React.FC<HomeViewProps> = ({ koreanDayNames }) => { 
  ...
  // ⭐️ 2. handleGoToFeedback 함수 수정: Journal ID 갱신 및 컴포넌트 렌더링으로 전환 ⭐️
  const handleGoToFeedback = (journal: JournalResponse) => {
    // ⭐️ 2a. 전역 상태에 journalId 갱신 ⭐️
    setCurrentJournalId(journal.journalId); 
    
    // ⭐️ 2b. 라우팅 대신 'grammar' 뷰로 전환 ⭐️
              ...
              {isTodaySelected ? (
                  ...
                  >
                      ...
                      <View style={styles.todayText}>
                          ...
                          <Text style={styles.todaySubtitle}>버튼을 눌러 첫 문장을 시작해보세요.</Text>
                      </View>
                  </TouchableOpacity>
              ) : (
                  <View style={styles.todayHeader}>
                      ...
                      <Text style={styles.loadingText}>선택된 날짜에 작성된 일기가 없습니다.</Text>
                  </View>
              )}
          </View>
      );
